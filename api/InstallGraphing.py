...
@app.callback(Output("elec_graph", "children"), Input("radios", "value"))
def update_graph(radio_value):
    dff = dfupdate(radio_value, df)
    fig = px.bar(dff, x="Service Months", y=radio_value, height=800, width=800)
    fig.update_layout(transition_duration=100)
    fig.update_layout(mapbox_style="open-street-map")
    fig.update_layout(margin={"r": 0, "t": 0, "l": 0, "b": 0})
    fig.update_layout(showlegend=True)
    graph = dcc.Graph(figure=fig, id="fig2test")
    return graph
